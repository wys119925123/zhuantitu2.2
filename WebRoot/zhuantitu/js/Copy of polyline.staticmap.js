...
LqUtil.get("thematicPolylineCategory_loadByMenuid",true,{'mid':mid},function(json){
	if(json.status){
...
function showBasePolyline(){
	...
	LqUtil.get("thematicPolyline_list",false,{'mid':mid,'floorid':zoneid + '00'},function(d){
		if(d.status){
			...
			$.each(d.data, function(i, v){
				...
				var polylineFeaure = new LMap.Feature.Vector(
						LMap.Geometry.fromWKT(polylineWkt),{
							gid:v.polylineid,
							strokewidth:v.strokewidth,
							strokecolor:"#"+v.strokecolor,
							title:v.name
...
function showFloorPolyline(floorid){
	...
	LqUtil.get("thematicPolyline_list",true,{'mid':mid,'floorid':floorid},function(d){
		if(d.status){
...
function deletePolyline(id){
	layer.confirm('你确定要删除吗？', {
		...
		fadeIn: 1000,
		yes: function(index){ 
		    LqUtil.post("thematicPolyline_delete",true,{"id":id},function(d){
		    	if(d.status){
...
function polylineFeatureselected(e){
	...
	LqUtil.get("thematicPolyline_detail",true,{'id':feature.attributes.gid},function(d){
		if(d.status){
			if(categoryColumnDefineMap.contains(d.categoryid)){
				...
				LqUtil.get("tableClumnDefine_loadPolylineByCid",true,{'id':d.categoryid},function(json){
					if(json.status){
...
function showEditPopup(){
	...
	temp.push("</select>" +
		...
		"<input type=\'hidden\' value=\'" + "LINESTRING" + editPolylineData.coordinates.replace('[[','(').replace(']]',')').replace(/\],\[/g,'-').replace(/\[/g,'').replace(/\,/g,' ').replace(/\-/g,',').replace(/\s+/g,"-") +"\' name=\'lonlat\' />"+
...
function generatingColumn(cid,data){
	...
	if(categoryColumnDefineMap.contains(cid)){
		...
		showLoadding();
		LqUtil.get("tableClumnDefine_loadPolylineByCid",true,{'id':cid},function(json){
			if(json.status){
				var columnDefineMap = new HashMap();
				$.each(json.data, function(i, v){
					columnDefineMap.put(v.columnName,{
