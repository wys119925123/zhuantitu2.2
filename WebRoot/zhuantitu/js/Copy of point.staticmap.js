...
function deleteMarker(id){
	layer.confirm('你确定要删除吗？', {
		...
		fadeIn: 1000,
		yes: function(index){ 
		    LqUtil.post("thematicPoint_delete",true,{"id":id},function(d){
		    	if(d.status){
...
function markerFeatureselected(e){
	...
	LqUtil.get("thematicPoint_detail",true,{'id':feature.attributes.gid},function(d){
		if(d.status){
			if(categoryColumnDefineMap.contains(d.categoryid)){
				...
				LqUtil.get("tableClumnDefine_loadByCid",true,{'id':d.categoryid},function(json){
					if(json.status){
...
function mapClick(e){
	...
	$('.add-marker-popup-ok').click(function(){
		showAddPopup(lonlat.lon,lonlat.lat);
	});
}
...
function generatingColumn(cid,data){
	...
	if(categoryColumnDefineMap.contains(cid)){
		...
		showLoadding();
		LqUtil.get("tableClumnDefine_loadByCid",true,{'id':cid},function(json){
			if(json.status){
				var columnDefineMap = new HashMap();
				$.each(json.data, function(i, v){
					columnDefineMap.put(v.columnName,{
