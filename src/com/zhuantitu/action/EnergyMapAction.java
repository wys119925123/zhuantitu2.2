...
public class EnergyMapAction extends BaseAction {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -8343377056517881768L;
	...
	public String bulidStatistics(){
		try {
			...
			StringBuffer flooridsBuf = new StringBuffer();
			if(StringUtil.isNotEmpty(floorids)){
				String [] floors = floorids.split(",");
				for (String str : floors) {
					flooridsBuf.append("'" + str +"',");
				}
			}
			PageBean<?> page = this.meterDayStatisticsService
				.pageByHql(StringUtil.deleteLastStr(flooridsBuf.toString()), locationid, metertype, startTime, flishTime,getPageSize(), getPage());
	...
	public String initData(){
		String nowTime = DateUtil.formatDatetime(System.currentTimeMillis() + (1000 * 60 * 60 * 24));
		List<ThematicPoint> thematicPoints = this.thematicPointService
				.getByHql("from ThematicPoint as t where t.thematicPointCategory.thematicMapMenu.menuid =  " + menuid + " ");
		...
		if(electricTargetId.length() > 0){
			String resultData = EnergyClient.statistics(StringUtil.deleteLastStr(electricTargetId.toString()),
					"Meter","Daily","total","60019","90001",nowTime);
			updateData(resultData,locationMap,floorMap,"50107");
			
			resultData = EnergyClient.statistics(StringUtil.deleteLastStr(electricTargetId.toString()),
					"Meter","Daily","total","60019","90001","2016-10-31 00:00:00");
			updateData(resultData,locationMap,floorMap,"50107");
			
		}
		if(waterTargetId.length() > 0){
			String resultData = EnergyClient.statistics(StringUtil.deleteLastStr(waterTargetId.toString()),
					"Meter","Daily","total","60023","90031",nowTime);
			updateData(resultData,locationMap,floorMap,"50205");
			
			resultData = EnergyClient.statistics(StringUtil.deleteLastStr(waterTargetId.toString()),
					"Meter","Daily","total","60023","90031","2016-10-31 00:00:00");
			updateData(resultData,locationMap,floorMap,"50205");
			
		}
		writeError(100, "初始化成功");
		return NONE;
	}
	private void updateData(String result,Map<String, Integer> locationMap, Map<String, String> floorMap,String metertype){
		...
		for (Object meterData : resultDataToJsonArr) {
			...
			for (Object object : meterDayResult) {
